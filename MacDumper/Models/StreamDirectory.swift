...
enum StreamType: UInt32, CaseIterable {
    case unused                    = 0
    case reserved0                 = 1
    case reserved1                 = 2
    case threadList                = 3
...
struct StreamDirectoryEntry: Identifiable {
    static let size = 12

    let id = UUID()
    let streamType: UInt32
    let dataSize: UInt32
    let rva: UInt32  // Relative Virtual Address (file offset)
...
struct StreamDirectory {
    ...
    let entries: [StreamDirectoryEntry]

    init?(from data: Data, header: MinidumpHeader) {
    ...
    func stream(ofType type: StreamType) -> StreamDirectoryEntry? {
        entries.first { $0.type == type }
    }

    func streams(ofType type: StreamType) -> [StreamDirectoryEntry] {
