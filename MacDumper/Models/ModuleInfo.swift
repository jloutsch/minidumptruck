...
struct ModuleVersion {
    let signature: UInt32  // 0xFEEF04BD
    let structVersion: UInt32
    let fileVersionHigh: UInt32
    let fileVersionLow: UInt32
    let productVersionHigh: UInt32
    let productVersionLow: UInt32
    let fileFlagsMask: UInt32
    let fileFlags: UInt32
    let fileOS: UInt32
    let fileType: UInt32
    let fileSubtype: UInt32
    let fileDateHigh: UInt32
    let fileDateLow: UInt32
...
struct ModuleInfo: Identifiable {
    ...
    let moduleNameRva: UInt32
    let version: ModuleVersion?
    let cvRecordLocation: MinidumpLocationDescriptor?
    let miscRecordLocation: MinidumpLocationDescriptor?

    var name: String = ""  // Populated after parsing
    var timestamp: Date { Date(timeIntervalSince1970: TimeInterval(timeDateStamp)) }
    ...
    func offset(for address: UInt64) -> UInt64? {
...
struct ModuleList {
    ...
    static let maxModules: UInt32 = 50_000
